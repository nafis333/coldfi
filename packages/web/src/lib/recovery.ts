// CryptoError replaced with plain Error for browser compatibility
import { encryptWithRawKey, decryptWithRawKey, uint8ArrayToBase64, base64ToUint8Array } from './crypto';
...
const WORDLIST: string[] = [
  ...
  'expose', 'extend', 'extra', 'eye', 'eyebrow', 'fabric', 'face', 'faculty',
  ...
  'place', 'planet', 'plastic', 'plate', 'play', 'player', 'pleasure', 'plenty',
  'plumber', 'pocket', 'poem', 'poet', 'poetry', 'point', 'poison', 'police',
  'policy', 'polo', 'pond', 'pony', 'pool', 'popular', 'portion', 'position',
  ...
  'such', 'sugar', 'suggest', 'suit', 'sun', 'sunny', 'sunset', 'super',
  'supply', 'support', 'suppose', 'sure', 'surface', 'surgery', 'surprise', 'surround',
...
export function generateRecoveryPhrase(): string {
  const entropy = crypto.getRandomValues(new Uint8Array(RECOVERY_WORD_COUNT * 2));
  const words: string[] = [];

  for (let i = 0; i < RECOVERY_WORD_COUNT; i++) {
    const idx =
      (entropy[i * 2] << 8 | entropy[i * 2 + 1]) % WORDLIST.length;
...
async function deriveRecoveryKey(
  ...
  const keyMaterial = await crypto.subtle.importKey(
    'raw',
    new TextEncoder().encode(phrase.normalize('NFKD')),
    'PBKDF2',
    false,
    ['deriveBits']
  );
...
export async function generateRecoveryBundle(
  ...
  try {
    ...
    const recoveryKey = await deriveRecoveryKey(phrase, userSalt);

    const encrypted = await encryptWithRawKey(recoveryKey, uint8ArrayToBase64(pek));

    const hashBuffer = await crypto.subtle.digest(
      'SHA-512',
      recoveryKey
    );
    const recoveryKeyHash = uint8ArrayToBase64(new Uint8Array(hashBuffer));

    return {
      recoveryPhrase: phrase,
      encryptedPek: encrypted.ciphertext,
      recoveryKeyHash,
    };
  } catch (error) {
    throw new Error(`Failed to generate recovery bundle: ${error instanceof Error ? error.message : String(error)}`);
  }
}

export async function recoverPEK(
  recoveryPhrase: string,
  userSalt: Uint8Array,
  encryptedPek: string
): Promise<Uint8Array> {
  try {
    const recoveryKey = await deriveRecoveryKey(recoveryPhrase, userSalt);
    const pekBase64 = await decryptWithRawKey(recoveryKey, encryptedPek);
    return base64ToUint8Array(pekBase64);
  } catch {
